...
export default function SpendingAnalytics() {
  ...
  useEffect(() => {
    ...
    supabase
      .from("orders")
      .select("cost, created_at, status, services(platform)")
      .eq("user_id", user.id)
  ...
  const stats = useMemo(() => {
    ...
    orders.forEach((o) => {
      const p = (o.services as any)?.platform || "Other";
      platformMap[p] = (platformMap[p] || 0) + Number(o.cost);
    });
    ...
    orders.forEach((o) => {
      ...
      if (dailyMap[day] !== undefined) dailyMap[day] += Number(o.cost);
    });
