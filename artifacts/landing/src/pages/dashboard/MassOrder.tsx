    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      try {
        const { error } = await supabase.functions.invoke("provider-proxy", {
          body: { action: "add", serviceId, link, quantity: parseInt(qty) },
        });
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="glass rounded-2xl p-6">
        ...
        <p className="text-sm text-muted-foreground mb-6">
          Submit multiple campaigns at once. One per line using the format:
        </p>
        <div className="glass rounded-lg p-3 mb-4 font-mono text-xs text-muted-foreground">
          ...
          {results.map((r) => (
            <div
              ...
              className={`flex items-center gap-3 text-sm p-2 rounded-lg ${
                r.status === "success" ? "bg-primary/10 text-primary" : "bg-destructive/10 text-destructive"
              }`}
            >
